...
import { Filiere, Student, type Role, Todo } from "./models";

export const fetchAllTodos = async (token: string) => {
  try {
    const resp = await axios({
      method: "get",
      url: `http://localhost:8080/api/v1/todos`, //`${import.meta.env.VITE_SOME_KEY}/api/v1/todos`,
      headers: {
...
export const deleteTodo = async (todo: Todo, token: string) => {
  try {
    await axios({
      method: "POST",
      url: `http://localhost:8080/api/v1/todos/${todo.id}`,
      headers: {
        'Authorization': `Bearer ${token}`
      }
    });
