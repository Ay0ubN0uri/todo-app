          ...
          <div className="flex w-full items-center justify-around px-4">
            <div>
              ...
              <nav
                ...
                id="navbarCollapse"
                className={`absolute right-4 top-full w-full max-w-[250px] rounded-lg  px-6 py-5 shadow dark:bg-dark-2 border-white border-[1px] md:border-[0px] lg:static lg:block lg:w-full lg:max-w-full lg:shadow-none lg:dark:bg-transparent ${!open && "hidden"
                  } `}
              >
                <ul className="block lg:flex">
                  <ListItem NavLink="/">Home</ListItem>
                  {isAuthenticated && (
                    <ListItem NavLink="/todos">Todos</ListItem>
                  )}
                </ul>
              </nav>
            </div>
            <div className="hidden justify-end pr-16 sm:flex lg:pr-0 space-x-3">
              {isAuthenticated && (
                <Button
                  onClick={() => {
                    logout();
